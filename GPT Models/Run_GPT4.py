...
print("Dinesh API Key", OPENAI_API_KEY)

if not OPENAI_API_KEY:
    raise ValueError("API key not found. Ensure it is set in the .env file")

root_path = '/home/dkarthikeyan/private/Causal_Project/causalbenchmark/eval/'
data_name = '/home/dkarthikeyan/private/Causal_Project/causalbenchmark/eval/cladder-v1/cladder-v1-q-balanced'

models_data_name = './cladder-v1/cladder-v1-meta-models'
with open(f'{models_data_name}.json', 'r') as f:
    import json
...
class DataFileList:
    def __init__(self, file_pattern=f'{data_name}.json'):
...
class Scorer:
    def __init__(self, files, data_list_of_dicts=None):
        ...
        if not all(key in data[0] for key in ['prompt', 'question_id']):
...
class Tester:
    ...
    def run_default_test(self, just_scoring=just_scoring, enable_cot=enable_cot):
        ...
        # Use only the gpt-3.5-turbo-0125 model
        ...
        #print(combs)
        #print("Dinesh Calling OpenAI API")
...
#                 openai_key_alias=OPENAI_API_KEY  # Use the environment variable key
#             )
            chat = Chatbot(
                model_version="gpt-4",
                max_tokens=max_tokens,
            ...
            #print("Dinesh pred lambda function created")
            ...
            #print(f"Creating write out file: {write_out_file}")
            
            os.makedirs(os.path.dirname(write_out_file), exist_ok=True)
            
            write_out_files.append(write_out_file)
            #print("Dinesh WOFS:", write_out_files)
            
            

            df_list = DataFileList()
            #print("Dinesh DFL Length check", df_list.data_objs)
            for data_file_obj in df_list.data_objs:
                #print("Nandita File object present")
                if just_scoring:
                    #print(" Nandita - just scoring loop")
                    ...
                    for datum_i, datum in tqdm(list(enumerate(data)), desc=tqdm_desc):
                        #print("Nandita loop works")
                        query = datum['prompt']
                        #print(f"Sending query to model: {query}")  # Debug statement before sending query
                        try:
                            pred = get_pred(query)
                            #print(f"Received prediction: {pred}")  # Debug statement after receiving prediction
                        ...
                        #print(f"Data written to file: {write_out_file}")  # Debug statement to confirm writing
                else:
                    #print("Dinesh TIFLLM called")
                    data_obj = TextInterfaceForLLMs(write_out_file)

                data_obj.response_processor(model_version=f"{model_version}{cot_suffix}")
                
        diff_time = time.time() - start_time
        print("Dinesh Calling Scorer Method", "Time: ", diff_time)
